...
export default {
  ...
  head () {
    return {
      title: this.$t('CommunicationObjects')
    }
  },
  meta: {
    ...
    cols () {
      return [
        ...
        },
        {
          ...
          show: true,
          formatter: (val) => {
            return val ? this.$t('Active') : this.$t('NotActive')
          }
        }
      ]
    }
  ...
  methods: {
    ...mapMutations({
      setPagination: 'communication-objects/setPagination'
    }),
    onChange () {
      this.isLoad = true
      ...
      this.$axios.$get('/communicationObjects', {
        params,
        headers: {
        ...
        .then((res) => {
          this.selectedId = 0
          this.rows = res?._embedded?.communicationObjects || []
