<template>
  <BaseSlider
    v-model="isOpen"
    :title="$t('CommunicationObjectsDelete')"
    :is-loading="isLoad"
  >
    <template #body>
      <FormBase
...
export default {
  name: 'CommunicationObjectsDelete',
  ...
  mounted () {
    ...
    this.$axios.$get('/communicationObjects?page=0&size=9999')
      .then((res) => {
        this.items = res?._embedded?.communicationObjects
      })
  ...
  methods: {
    ...
    send () {
      if (this.selectedId !== 0) {
        this.isLoad = true
        this.$axios.$delete('/communicationObjects/' + this.selectedId, {
          headers: {
            Authorization: true
          }
        })
